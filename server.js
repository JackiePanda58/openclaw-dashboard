...
function getSystemStats() {
  try {
    ...
    // Calculate real CPU usage by comparing CPU times
    let cpuUsage = 0;
    try {
      const cpus = os.cpus();
      const now = Date.now();
      ...
      if (lastCpuInfo && lastCpuTime) {
        ...
        if (totalTick > 0) {
          cpuUsage = Math.min(Math.round((1 - totalIdle / totalTick) * 100), 100);
        }
      }
      
      // Cache current CPU info for next reading
      lastCpuInfo = JSON.parse(JSON.stringify(cpus));
      lastCpuTime = now;
    } catch {
      cpuUsage = 0;
    }
